...
\section{Flare Rates Compared to P$_{rot}$}

\begin{figure}[ht!]
    \script{prot_histograms.py}
    \begin{centering}
        \includegraphics[width=\linewidth]{figures/prot_histograms.pdf}
        \caption{
            Rotation periods and flare rates for M, K, and G stars (top to bottom).
        }
...
stars with ages $\pm 30$\,Myr of the planet hosting star and $T_\textrm{eff} \pm 1000$\,K. We calculate the overall
flare rate as described in \cite{Feinstein22}:

\begin{equation}
  \mathcal{R} = \frac{1}{\tau_\textrm{obs}} \left( \sum_{i=1}^{N} p_i \right)
\end{equation}

where $\mathcal{R}$ is the flare rate in day$^{-1}$, $\tau_\textrm{obs}$ is the total amount of time a target was
observed with TESS, and $p_i$ is the probability that flare $i$ is a true flare as assigned by \texttt{stella}. We present
